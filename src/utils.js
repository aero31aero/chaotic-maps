...
let lyapunov = function () {
	let calc = (opt) => {
		...
		let exp = (1/(count+1)) *  Math.log(Math.abs(sum / (x2_orig - x1_orig)));
		return exp;
	};
	return calc;
}();
...
var img_utils = function () {
	let load_image = () => {
		...
		c2.getContext('2d').clearRect(0,0,img.width,img.height);
		c3.getContext('2d').clearRect(0,0,img.width,img.height);
